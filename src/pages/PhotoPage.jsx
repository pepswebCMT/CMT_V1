...
const PhotoPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          const storageRef = ref(storage, `images/${Date.now()}`); // Utiliser un timestamp pour nommer l'image
          const uploadTask = await uploadBytes(storageRef, file);
          const imageUrl = await getDownloadURL(uploadTask.ref);
          ...
          // Ajouter automatiquement un document avec un ID unique
          const docRef = await addDoc(collection(db, "PendingTombs"), tombData);
  ...
  return (
    <>
      <Navbar />
      <section className="w-full flex justify-center items-center font-josefin">
        <div className="w-full max-w-96 flex flex-col justify-between gap-6 items-center p-2">
          <motion.div className="w-full top-28 z-40 bg-white p-2 flex flex-col justify-center items-center gap-3 font-josefinBold font-bold rounded-xl">
            <Link
              to="#"
              onClick={() => navigate(-1)}
